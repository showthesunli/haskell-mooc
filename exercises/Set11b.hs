...
appendAll ior [] = do
  str <- readIORef ior
  putStrLn str
appendAll ior (s : ss) = do
  remind <- readIORef ior
  writeIORef ior $ remind ++ s
  appendAll ior ss
...
hSelectLines h nums = do
  ...
  return (map ((contents !!) . subtract 1) (sort nums))
