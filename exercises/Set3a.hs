...
maxBy :: (a -> Int) -> a -> a -> a
maxBy measure a b = if x > y then a else b
  where
    x = measure a
    y = measure b
...
capitalizeFirst "" = ""
capitalizeFirst (s : ss) = toUpper s : ss
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (<= max) [k ^ y | y <- [0 ..]]
...
joinToLength len ss = [sub | c <- ss, d <- ss, let sub = c ++ d, length sub == len]
...
sumRights [] = 0
sumRights (Left err : es) = sumRights es
sumRights (Right v : es) = v + sumRights es
